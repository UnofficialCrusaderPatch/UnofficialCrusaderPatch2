...
namespace UCP.Patching
{
    public class BinaryEdit : BinaryEditBase
    {
        ...
        public BinaryEdit(string blockIdent)
        {
            ...
            string file = string.Format("UCP.Resources.CodeBlocks.{0}.block", blockIdent);
        ...
        protected override bool GetAddresses(byte[] original, out int rawAddr, out int virtAddr)
        {
            ...
            if (count > 1)
            {
                Installer.AddFailure(this.blockFile, EditFailure.MultipleBlocks);
                return false;
            }
            else if (count == 0)
            {
                Installer.AddFailure(this.blockFile, EditFailure.BlockNotFound);
                return false;
            }
            return true;
        }
